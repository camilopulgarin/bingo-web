...
const BingoTableMock = ({ userId, gameId, tableId, card, onBingo }) => {
  ...
  useEffect(() => {
    const handleBingoResult = (result) => {
      console.log("📩 Resultado recibido:", result, "para tabla:", tableId);
      // 🔹 Mostrar mensaje solo si esta tabla fue la última que presionó “Bingo”
      if (lastBingoRequest === tableId) {
        setBingoResult(result);
      }
    };

    socket.on("bingoResult", handleBingoResult);
    return () => socket.off("bingoResult", handleBingoResult);
  }, [tableId]);
  ...
  const handleBingo = () => {
    ...
    socket.emit("bingo", { userId: userId, boardId: tableId, gameId: gameId });
    if (onBingo) {
      ...
      {bingoResult && (
        <div
          className={`mt-2 text-center font-bold ${
            bingoResult.success ? "text-green-700" : "text-red-700"
          }`}
        >
          {bingoResult.message}
        </div>
